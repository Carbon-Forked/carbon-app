...
export const XAxis = () => {
  ...
  const xTicks = useMemo(() => {
    const validDomain = xScale
      .domain()
      .map(Number)
      .filter((d) => !isNaN(d));
    if (!validDomain.length) return [];

    const length = validDomain.length;
    ...
    const ticks = validDomain.filter((_, i) => i % m === m - 1);
    return ticks;
  }, [dms.boundedWidth, xScale, zoom]);

  const ticks = xTicks.map((tickValue) => {
    const x = (xScale(String(tickValue)) ?? 0) + xScale.bandwidth() / 2;
    return (
